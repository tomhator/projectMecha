...
- `Resources/`: 데이터 모델 스크립트(`.gd`) 및 인스턴스(`.tres`).
    ...
    - `Skills/`: 스킬 데이터 리소스 (`.tres`). 저격 3종(`skill_snipe_arm` 등)·수집가 전용 스킬 포함.
    - `Test/`: 개발용 테스트 리소스.
...
코어는 `core_base.tres` 1개로 고정. 런 시작 전 어빌리티 트리에서 포인트(런당 3pt)를 소비해 빌드를 구성한다.
- **3분기:** 공격(공격배율↑) / 방어(HP·쉴드↑) / 기동(행동력↑)
- **코어 스킬:** 분기 3노드 완주 시 해당 분기의 코어 스킬 언락 (`GameState.active_core_skill`)
- **상세:** [[AbilityTreeSystem]]

### 3.2 턴제 전투 시스템 (Turn-based Combat)
- **교대식 턴제:** 플레이어 턴과 적 턴이 번갈아 진행됩니다.
- **스킬 기반 행동:** 부품에 할당된 **SkillData**와 트리에서 언락된 코어 스킬을 사용하여 전투합니다.
