...
Codex에서는 아래 글로벌 스킬을 우선 사용한다. 설치 위치는 기본적으로 `~/.codex/skills`다.

| 스킬 | 용도 |
|------|------|
| `project-mecha-workflow` | 프로젝트 공통 작업 절차, 스펙 리뷰, WorkNote/TODO 갱신 |
| `project-mecha-gdscript-guard` | Godot 4 GDScript 문법·검증 기준 확인 |
| `project-mecha-combat-debug` | 턴제 전투, AP, 타겟팅, 파츠 파괴, 수집가 보스 조사 |
| `project-mecha-skill-resource` | 게임 `SkillData` 리소스 추가·수정 |
| `project-mecha-enemy` | 역할 기반 적 리소스와 적 스킬 추가·수정 |
| `project-mecha-parts-update` | 파츠, affix, 손상도, `EXTRA_ARM` 관련 수정 |

Claude Code에서는 같은 반복 작업을 `.claude/commands/`의 커맨드 문서로 수행한다. 도메인별 작업 전에는 `Scenes/Combat/CLAUDE.md`, `Scenes/Entities/CLAUDE.md`, `Scripts/Autoload/CLAUDE.md`, `Resources/Skills/CLAUDE.md` 중 해당 파일을 함께 읽는다.
...
- `.claude/commands/`: 반복 작업 절차를 Claude Code용 커맨드 문서로 제공한다.
