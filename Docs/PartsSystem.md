...
> [!note]
> 데모 코어는 `core_base.tres` 1종이다. 과거 3코어 하중 구분은 어빌리티 트리와 LEG 파츠 조합으로 대체한다.
...
| 조합 | 최대 하중 |
|------|---------|
| 기본 코어 + 고속 역관절 2각 | 100 + 5 = **105** |
| 기본 코어 + 중장 무한궤도 | 100 + 30 = **130** |
...
| 스킬명 | 설명 | 등장 티어 |
|--------|------|---------|
| **파츠 저격** | 지정 슬롯 손상도 -1. 다음 턴 행동 예고로 미리 공개. | Breaker 병종 |
| **과부하 공격** | 일반 공격 + 피격 슬롯 손상도 -2. 대미지 낮음. | 후반 Breaker, 보스 |
| **EMP 충격** | 대미지 없음. 장착된 모든 파츠 손상도 -1. | 보스 또는 지역 특수 적 |
...
전투가 끝나도 손상도는 **자동 회복되지 않는다.** 모든 파츠의 `durability`는 전투 종료 시점 값 그대로 유지된다.

> [!important]
> 손상도는 런 전체에 걸친 자원 관리 요소다.  
> 수리를 언제 할지, 어느 파츠를 우선할지가 플레이어의 결정이 된다.

### 6.5 수리

| 수단 | 효과 | 시점 |
|------|------|------|
| 현장 수리 키트 (백팩 파츠 스킬) | 선택 파츠 1개 손상도 max 복구 | 전투 중 |
| 작업대 | 크레딧 소모 → 선택 파츠 1개 손상도 max 복구 | 던전 중 |
| 조우 이벤트 | 이벤트에 따라 손상도 +1~max 복구 | 던전 중 |
| 거점 격납고 | 고철 소모 → 선택 파츠 수리 | 런 종료 후 |
