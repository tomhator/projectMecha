...
- [ ] **첫 런 직접 플레이** — Godot 실행 → 1층 오른팔 획득·장착 → 2층 오른팔 재드롭 → "갈면 1층 거 파손" 딜레마가 실제로 체감되는지 확인 (= 아래 P0.5 §V2). 이어서 V1·V3~V6을 5런 플레이테스트로 측정.
  - **(2026-06-01 헤드리스 세션)** 층위 1 기능 검증은 `validate.sh` 전체 PASS(Godot 4.6.2)로 자동 보증함 (V5 빈도 평균 9.15회/런, 0회 런 0/200 PASS). **남은 것은 사람 관찰 영역(V1·V2·V3·V4·V6)** — 로컬 Godot 4.6에서 사용자가 직접 플레이테스트해야 함.

### 2순위 — 누적 UI 수동 QA
- [ ] **거점 UI 플레이테스트** — Hub/Hangar 버튼 위치, 카드 텍스트 overflow, 출격 준비 흐름, RunEnd 정산 문구를 실제 화면에서 확인·조정한다.
