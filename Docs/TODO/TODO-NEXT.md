...
- [ ] **첫 런 직접 플레이** — Godot 실행 → 1층 오른팔 획득·장착 → 2층 오른팔 재드롭 → "갈면 1층 거 파손" 딜레마가 실제로 체감되는지 확인 (= 아래 P0.5 §V2). 이어서 V1·V3~V6을 5런 플레이테스트로 측정.
  - **(2026-06-01 헤드리스 세션)** 층위 1 기능 검증은 `validate.sh` 전체 PASS로 자동 보증함 (V5 빈도 평균 6.20회/런, 0회 런 0/200 PASS). **남은 것은 사람 관찰 영역(V1·V2·V3·V4·V6)** — 로컬 Godot 4.6에서 사용자가 직접 플레이테스트해야 함.
...
- [x] **V5 런당 교체 딜레마 3회+ / 4층 연속 0회 없음** — headless 측정(`check_swap_dilemma_frequency.gd`) 평균 6.20회/런(목표 ≥3.0), 0회 런 0/200 → PASS (2026-06-01). (사람 플레이테스트 5런 집계는 V1·V6과 함께 별도 수행 권장)
