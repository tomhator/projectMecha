...
- [ ] **HubScene v1 구현** — 외곽 은신처 메인 화면과 건물형 버튼을 만들고, 프로젝트 시작/런 종료 복귀 흐름을 거점으로 연결한다.
...
- [ ] **HubScene v1** — `Scenes/Base/HubScene.tscn` / `.gd` 추가, 은신처 마당 건물형 버튼, 기존 코어 연구·조립 화면 연결, 런 종료 복귀 대상 변경
- [ ] **거점 영구 상태 구현** — 영구 파츠 창고, 고철, 성공/실패 정산(실패 시 장착 파츠 영구 손실, 런 인벤토리 회수, 재화 50% 회수)
- [ ] **격납고 서비스 v1** — 파츠 수리(고철 소모), 파츠 분해(고철 획득), 출격 전 장착/런 인벤토리 구성
