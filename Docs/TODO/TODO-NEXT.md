...
> **2026-06-08 세션 요약:** 코어 전환 문서 부채를 정리했다 — GDD §7.3 작업대를 승인된 재설계(수리·분해·제작 3탭)로 동기화, 스크랩→고철(커먼/레어) 자원 통합, **검증기준 nV1~nV7 확정**(§1.1), 전투 spec 두 교리 재검토 완료.
...
> 어제 전투 화면, 오늘 조립 화면 설계 완료. 같은 ItB×DD 규칙 + 새 코어("파츠가 주인공")로 나머지를 설계한다. (상세는 아래 "여유 시 — UI/UX 전면 개편" 참조)
- [ ] **공급 / prep 허브 설계** — `[맵][조립][공급]` TabNav. 새 코어=throughput이라 공급<소비면 파츠 굶주림(실패모드 ⑤′). 결정: 가용성(가벼운 상시 + 게이트 스파이크), 공급 메커니즘(제작·분해 루프 1개 추천, 상점·제작 동시 금지). → office-hours 다음 패스.
- [ ] **나머지 화면 설계** — 거점 마당 · 격납고 · 코어 설계 · 던전 맵 · 보상 · 상자 · 조우 · 작업대 · 결과.
...
- [ ] **UI/UX 전면 개편 — 화면별 설계 단계** (2026-06-01 office-hours로 첫 화면 설계 착수)
  ...
  - [ ] **전투 사이 prep 허브 + 공급 (다음 설계 패스)** — `[맵][조립][공급]` TabNav. 새 코어=throughput이라 공급<소비면 실패모드 ⑤(파츠 고갈) 회귀. 결정거리: 가용성(가벼운 상시+게이트 스파이크), 공급 메커니즘(**제작/분해 루프 1개 추천**, 상점·제작 동시 구현 금지=스코프).
  - [ ] **나머지 화면 설계** — 같은 §2 규칙(ItB×DD)으로: 거점 마당 · 격납고 · 코어 설계 · 던전 맵 · 보상 · 상자 · 조우 · 작업대 · 결과. 각 화면 "무대(DD)/계기판(ItB)" 구분 + "이유" 명시.
