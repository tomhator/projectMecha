...
> **2026-06-09 세션 요약:** 교리 노드 런-한정 boon 방향을 합의했다 — 하데스 풀카피 아님, 맵 갈림길 중 일부가 교리 패시브 부여(교리 노드=전투/조우/상자), 같은 교리 반복 시 **약한 시너지**, boon = **미리 세팅해온 코어 어빌리티 강화**(새 인터랙트 없음). 코어 어빌리티 현황 진단 결과 **교리가 코드에 0건·트리 3축 vs boon 2교리 미정합** 발견 → boon은 **선행 의존성(갭① 2교리 코어 어빌리티 정의)** 을 가짐. **boon 본 설계는 다음 세션으로 이월.** 재개 지침: [[SESSION_CONTEXT/boon-설계-2026-06-09]].
...
- [ ] **boon 본 설계 (이월)** — 방향은 06-09 합의 완료(코어 어빌리티 강화형·교리 노드=전투/조우/상자·약한 시너지). **선행 의존성: 갭① "2교리 vs 3축" 결정 → 교리 태그 코드 명문화.** 이후 노드 종류별 강화 매핑·시너지 기계·개별 boon. 재개 순서·3안 검토는 [[SESSION_CONTEXT/boon-설계-2026-06-09]] "다음 세션 재개 지침". 맵 스펙 §8.3/§9 미결.
...
- [ ] **나머지 화면 설계** — 거점 마당 · 격납고 · 코어 설계 · 보상 · 상자 · 조우 · 결과. (전투·조립·작업대·던전맵 완료)
