...
- [ ] **UI/UX 전면 개편 — 화면별 설계 단계** (2026-06-01 office-hours로 첫 화면 설계 확정)
  - **기준 문서:** [[superpowers/specs/2026-06-01-전투화면-코어어빌리티-재설계-design]] — 비주얼 규칙(ItB×DD) + 코어 어빌리티 재정의 + 전투 화면.
  - **순서 원칙(사용자 확정):** *모든 화면 설계 완료 후* 화면별 상세 구현 착수. 지금은 설계 단계.
  - [x] **전투 화면** 설계 완료 (2026-06-01).
  - [ ] **나머지 화면 설계** — 같은 §2 규칙(ItB×DD)으로: 거점 마당 · 격납고 · 코어 설계 · 던전 맵 · 조립 · 보상 · 상자 · 조우 · 작업대 · 결과. 각 화면 "무대(DD)/계기판(ItB)" 구분 + "이유" 명시.
  - [ ] 전 화면 설계 후 → **디자인 규칙 정식 문서**(팔레트·타이포·간격 토큰) 추출 → 화면별 구현(🟢 비주얼 먼저, 🔒 시스템은 V6 후).
