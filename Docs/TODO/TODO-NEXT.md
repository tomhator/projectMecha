...
> ⚠️ **옛 1순위(V1~V6 플레이테스트)는 보류** — 코어 전환으로 V1~V6이 무효가 됐다. 새 검증 기준(nV1~nV7) 확정 후 재개.
...
- [ ] **새 검증 기준(nV1~nV7) 확정** — 현재 검증기준 §1에 draft v2만 있음. 최종 metric·측정법 확정 (코어 전환으로 V1~V6 무효).
...
- [ ] **UI/UX 전면 개편 — 화면별 설계 단계** (2026-06-01 office-hours로 첫 화면 설계 착수)
  ...
  - [ ] **코어 전환 문서 정리** — GDD §17·§3.1·§4.4~4.6("교체=파손 비용" 프레임 제거), 검증기준 §0·V1~V6·실패모드 ①②⑤ 재작성(**V1 수락률 30~70% 밴드 무효**), 전투 문서 두 교리(정비공/해체공) 재검토.
...
> ⚠️ **(2026-06-03 코어 전환) 이 절의 V1~V6은 옛 코어 "교체의 긴장" 기준 — 폐기.** 새 코어 "파츠가 주인공", 새 검증 방향은 검증기준 문서 §1 draft v2. V1(수락률 밴드) 무효. 재검증은 새 기준 확정 후.
