...
| ID | 트리거 | 적용 로직 |
|----|--------|---------|
| `evolution_lord` | `on_equip` | 정상 BACK/ARM 파츠 장착 시 `EXTRA_ARM` 슬롯 추가. 제공 파츠가 파괴·탈착·강탈되면 추가 슬롯 파츠를 즉시 탈착 |
...
| `backdoor` | `conditional` | 현재 타겟 적에게 **디버프**가 1개 이상 있으면 `bonus_sum += +0.25`. 멀티타겟 스킬은 대상 중 하나라도 디버프가 있으면 총 피해에 1회 적용. 디버프 = BURN(화상), 행동력 감소, 그 외 추가 확정된 상태이상 |
