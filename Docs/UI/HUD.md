...
- 우측 하단: 선택 상세 패널. 인벤토리 파츠/장착 슬롯/스킬 프리뷰 선택에 따라 이름, 수치, 내구도, affix를 표시한다. (교체는 무비용 — 옛 "파손 비용" 표시 제거, 2026-06-03 코어 전환)
...
- 교체(무비용): 던전 중 장착 해제·교체는 비용이 없다(파츠 그대로 인벤토리로). 파손은 적 공격(내구도 0)·의도적 소비(투척·희생)로만 발생한다. (2026-06-03 코어 전환)
...
- 전투 보상: 격파 수, 드롭 개수, 크레딧/고철 획득량 표시.
- 전투에서 드롭된 파츠는 보상 화면 진입 시 모두 런 인벤토리에 자동 추가.
...
- 중앙 결과 텍스트 + 버튼 **「은신처로 복귀」** → HubScene v1의 외곽 은신처 메인.
- HubScene 구현 전 임시 연결은 CoreSelectScene으로 허용하되, 최종 데모 흐름은 [[BaseSystem]] 기준으로 거점 복귀를 사용한다.
